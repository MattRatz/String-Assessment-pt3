...
bool StringUtil::stringEqualTo(StringUtil a)  
{
	string testName = "EqualTo"; 
	bool testBool;
	if (a.aString == aString)
	{
		cout << "These 2 strings (" << a.aString << ", " << aString << ") are the same!" << endl;
		cout << endl;
		testBool = true;
		testUtil(testName, testBool);
		return true;
	...
	{
		cout << "These strings (" << a.aString << ", " << aString <<") are not equal!" << endl;
		cout << endl; 
		testBool = false; 
		testUtil(testName, testBool);
		return false; 
...
//void testAll
//{
// StringUtil myStringA("Hello");
// StringUtil myStringB("Hello");
// 
// myStringA.stringEqualTo(myStringB);
// 
// 
//}

string StringUtil::appendString(StringUtil a)
{
	bool testBool;
	string testName = "Append"; 
	string appendedString;
	cout << "Current string before appending : " << aString << endl;
	cout << "String to add: " << a.aString << endl; 
	appendedString = aString + a.aString;
	cout << appendedString << endl;
	cout << " " << endl;
	if (appendedString != aString + a.aString)
	{
		testBool = false;
	}
	else
...
string StringUtil::prependString(StringUtil a)
{
	bool testBool;
	string testName = "Prepend"; 
	string prependedString;
	cout << "Current string before prepend: " << aString << endl;
	cout << "String to be added: " << a.aString << endl; 
	prependedString = a.aString + aString; 
	cout << prependedString << endl; 
	cout << " " << endl;
	if (prependedString != a.aString + aString)
	{
		testBool = false; 
	}
	else
...
{
	int length = a.aString.length(); 
	cout << "Searching and replacing for the string " << b.aString << " in the string " << aString << endl;
	bool testBool = true;
	bool loopBool = true; //bool to control loop 
	string testName = "ReplaceString";
	size_t findPosition = aString.find(b.aString);
	do
	{
		size_t findPosition = aString.find(b.aString); //searching for "weep" in "weepwoopweep"
		if (findPosition == string::npos) 
		{
			loopBool = false; 
		...
		if (findPosition != string::npos) 
		{
			...
			aString.replace(findPosition, length, a.aString);
			cout << "The word is now changed to, " << aString << endl;
			testBool = true;
		}
		else 
		{
...
void StringUtil::runStringUtil() //function to run all functions
{
	...
	StringUtil append("appended");
	StringUtil prepend("prepended");
	StringUtil replacing("woop");
	...
	startString.stringEqualTo(HelloWorld); // comparing strings
	startString.appendString(append);
	startString.prependString(prepend);
	startString.toLower();
	startString.toUpper();
	...
	stringToSearch.replaceString(replacing, finding); //replaces string with set word
